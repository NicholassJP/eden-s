<section id="section-3">
    <div id="bg-section-3" class="d-flex align-items-center">
        <div class="container">
            <div class="text-center">
                <h1>Our Expertise</h1>
                <div class="row">
                    <div class="col-lg-6 mt-4">
                        <div class="card-section justify-content-center">
                            <img class="img-fluid" src="/img/section3/bg4.png">
                            <h1 class="m-0 absolute-center">MINING</h1>
                        </div>
                    </div>
                    <div class="col-lg-6 mt-4">
                        <div class="card-section justify-content-center">
                            <img class="img-fluid" src="/img/section3/bg3.png">
                            <h1 class="m-0 absolute-center">LEGAL</h1>
                        </div>
                    </div>
                    <div class="col-lg-6 mt-4">
                        <div class="card-section justify-content-center">
                            <img class="img-fluid" src="/img/section3/bg2.png">
                            <h1 class="m-0 absolute-center">IT SUPPORT</h1>
                        </div>
                    </div>
                    <div class="col-lg-6 mt-4">
                        <div class="card-section justify-content-center">
                            <img class="img-fluid" src="/img/section3/bg1.png">
                            <h1 class="m-0 absolute-center">FUNDING & INVESTMENT</h1>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>